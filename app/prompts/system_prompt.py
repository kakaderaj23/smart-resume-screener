...
def get_system_prompt() -> str:
    ...
    return (
        ...
        "1. Strictly Evidence-Based: Base your evaluation on BOTH the Structured CandidateProfile AND the Original Resume Text provided. If structured extraction misses information but the raw resume clearly contains the evidence, use the raw resume. Never ignore explicit evidence in the resume. Do not speculate or infer qualifications that are not explicitly stated or documented in either source.\n"
        "2. Zero Hallucinations: If details (such as skill expertise level, education, or duration of experience) are not present in either the supplied candidate profile or the original resume text, treat them as missing. Never invent or assume candidate history.\n"
